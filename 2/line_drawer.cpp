...
void line_drawer::draw_line() {
    bool swapped = abs(from.get_x() - to.get_x()) < abs(from.get_y() - to.get_y());
    if (swapped) {
        from.swap_axis();
        to.swap_axis();
        // std::swap(dx, dy);
    ...
    double dx = to.get_x() - from.get_x(), dy = to.get_y() - from.get_y();
    double gradient = dy / dx;
    double ht = thickness / 2;
    // std::cout << from.get_x() << ' ' << from.get_y() << ' ' << to.get_x() << ' ' << to.get_y() << ' ' << ht << ' ' << dx << ' ' << dy << ' ' << gradient << '\n';
    if (dy == 0) {
        gradient = 1;
        std::cout << "Drawing vertical or horisontal line\n";
        for (int x = static_cast<int>(round(from.get_x())); x <= static_cast<int>(round(to.get_x())); ++x) {
            // std::cout << "\nx = " << x << '\n';
            for (int y = static_cast<int>(round(from.get_y() - ht)); y <= static_cast<int>(round(from.get_y() + ht)); ++y) {
                // std::cout << y << ' ';
                pic.fill_pixel(x, y, swapped, brightness, 1, sRGB, gamma);
            }
        }
        return;
    }

    double curr_x = from.get_x(), curr_y = from.get_y() + gradient;
    draw_part_of_line(static_cast<int>(round(curr_x)), static_cast<int>(round(to.get_x())),
                      curr_y - ht, gradient, swapped);
    if (check_equals(thickness, 1)) {
        return;
    }
    draw_part_of_line(static_cast<int>(round(curr_x)), static_cast<int>(round(to.get_x())),
                      curr_y + ht - 1, gradient, swapped);
    if (check_equals(thickness, 2)) {
        return;
    }

    while (curr_x <= to.get_x()) {
        for (int y = static_cast<int>(round(curr_y - ht + 1)); y <= static_cast<int>(curr_y + ht - 1); ++y) {
            pic.fill_pixel(static_cast<int>(round(curr_x)), y, swapped, brightness, 1, sRGB, gamma);
        }
        // pic.fill_pixel(static_cast<int>(round(curr_x)), curr_y + ht - 1, brightness, 1, sRGB, gamma);
        // pic.fill_pixel(static_cast<int>(round(curr_x)), curr_y - ht + 1, brightness, 1, sRGB, gamma);
